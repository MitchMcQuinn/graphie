"""
core/store_memory.py
----------------
The store_memory.py script provides a focused utility for storing the outputs of workflow steps in Neo4j SESSION nodes. It manages the persistence of function outputs and error information in the graph database.

This module has a specific, targeted purpose within the graph-based workflow system:
- Output Storage: It stores the output data from workflow steps into SESSION nodes in Neo4j.
- History Tracking: It maintains a history of outputs for each step by storing them as arrays, with each new output appended as a new "cycle."
- Error Logging: It can optionally store error information that occurred during step execution.
- Session State Persistence: It contributes to the overall persistence mechanism that allows the workflow to survive server restarts.
